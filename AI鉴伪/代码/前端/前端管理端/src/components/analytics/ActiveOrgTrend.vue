...
const fetchChartData = async () => {
    try {
        const res = await analyticsApi.getDailyActiveOrganizations()
        const data = res.data

        const dates = data.map((item: any) => item.date)
        const organizations = data.map((item: any) => item.organization_count)

        renderChart({ dates, organizations })
    } catch (err) {
        console.error('加载活跃组织数据失败:', err)
        snackbar.showMessage('加载活跃组织数据失败')
    }
}
