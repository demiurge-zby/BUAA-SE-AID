...
def get_data(hyper_para, config, split='train'):
    ...
    dataset_dict = {
        ...
        "BioFors_train": "BioFors_train",
        "BioFors_test": "BioFors_test",
    ...
    if split == 'train':
        dataset = get_dataset_by_name(config, hyper_para, dataset_name, split='train')
        batch_size = hyper_para.train.batch_size
        data_loader = get_dataloader(dataset, 'train', batch_size)
    elif split == 'test':
        dataset = get_dataset_by_name(config, hyper_para, dataset_name, split='test')
...
def get_dataset_by_name(config, hp, dataset_name, sample_num=-1, split='train'):
    txt_name = f'{dataset_name}.txt'
    return BioDataset(config, osp.join(config.bio_data_txt_dir, txt_name),
                      hp.data.img_size, split=split)
